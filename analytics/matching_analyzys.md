...
Задача алгоритма мэтчинга — находить пары между лидами, которые клиентами, которые ищут квартиру и теми, кто её продаёт.  
...
Основная идея — не усложнять MVP.  
Базовый мэтчинг должен сочетать фильтры по полям и семантическое сравнение описаний.  
Предрасчёт сходств позволяет убрать требования по высокой скорости на сервере и сделать систему устойчивой.  
Когда появятся данные об активностях (лайках и успешных сделках), можно будет перейти к обучаемым моделям и персонализации.